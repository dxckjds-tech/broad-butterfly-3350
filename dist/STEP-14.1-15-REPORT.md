...
- DeepSeek `ANALYZE_PRODUCT` MIC + VEMIC：**PASS**（identity / facts / health / 无 PII 泄漏）
- DeepSeek `translation`：**PASS**（`stainless steel ball valve` → `不锈钢球阀`，router 选 deepseek）
- DeepSeek `connection_test`：**PASS**
- Kimi：**adapter implemented / real API pending**（无 `KIMI_API_KEY`）
- OpenAI：**adapter implemented / real API pending**（无 `OPENAI_API_KEY`）
- Anthropic / Gemini / Qwen：**adapter implemented / real API pending**

自动路由真实双 Provider 对比未做（环境只有 DeepSeek）。Router mock 已覆盖 DeepSeek+OpenAI 的 vision / economy / quality。

## 15 DeepSeek/Kimi 兼容结果

旧 DeepSeek / Kimi Key 经迁移后无需重填即可进入 `providerConfigs`。  
仅配置 DeepSeek Flash 时，带图片的商品页回退为纯文本诊断，不会因为缺 Vision 而拒绝分析；若同时配置了视觉模型，则优先选视觉模型。
