const DEFAULTS = { provider: 'deepseek', deepseekBaseUrl: 'https://api.deepseek.com', deepseekModel: 'deepseek-v4-flash', deepseekThinking: 'disabled', kimiBaseUrl: 'https://api.moonshot.cn/v1', kimiModel: 'kimi-k2.5' };
...
async function callAI(messages, maxTokens = 4200) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const retryMessages = attempt === 0 ? messages : [...messages, { role: 'user', content: `上一次响应${lastReason}。请立即重新输出完整、非空、可解析的 JSON 对象，不要使用 Markdown 代码块。` }];
    const body = { model, messages: retryMessages, max_tokens: isK3 ? Math.max(maxTokens, 6000) : maxTokens, temperature: isKimi ? 1 : 0.2 };
    ...
    const requestTimeout = setTimeout(() => controller.abort(), Math.min(isK3 ? (attempt === 0 ? 90000 : 30000) : (attempt === 0 ? 35000 : 15000), remaining));
    let response;
    try { response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` }, body: JSON.stringify(body), signal: controller.signal }); }
    catch (error) { clearTimeout(requestTimeout); if (error.name === 'AbortError') { lastReason = '请求超时'; continue; } throw error; }
    ...
    const content = (typeof rawContent === 'string' ? rawContent : Array.isArray(rawContent) ? rawContent.map(part => part?.text || part?.content || '').join('') : '').trim();
    if (!content) {
      const reasoning = typeof message.reasoning_content === 'string' ? message.reasoning_content.trim() : '';
      if (reasoning) { const start = reasoning.indexOf('{'), end = reasoning.lastIndexOf('}'); if (start >= 0 && end > start) { try { return { result: sanitizeModelEvidence(JSON.parse(reasoning.slice(start, end + 1))), usage: data.usage || null, model: data.model || model, provider: providerName, attempts: attempt + 1 }; } catch {} } }
      lastReason = `为空${lastFinishReason ? `（finish_reason: ${lastFinishReason}）` : ''}`; continue;
    }
    const cleaned = content.replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '').trim();
    try { return { result: sanitizeModelEvidence(JSON.parse(cleaned)), usage: data.usage || null, model: data.model || model, provider: providerName, attempts: attempt + 1 }; }
    catch {
      const start = cleaned.indexOf('{'), end = cleaned.lastIndexOf('}');
      if (start >= 0 && end > start) {
        try { return { result: sanitizeModelEvidence(JSON.parse(cleaned.slice(start, end + 1))), usage: data.usage || null, model: data.model || model, provider: providerName, attempts: attempt + 1 }; } catch {}
      }
      lastReason = '不是有效 JSON';
    }
  }
  throw new Error(`${providerName} 未返回有效最终内容${lastFinishReason ? `（finish_reason: ${lastFinishReason}）` : ''}。${isK3 ? 'K3 已等待最多 125 秒，请检查账号额度或改用 K2/非思考模型' : '请重试或切换模型'}`);
}
...
async function listAIModels() {
  ...
  return { provider: providerName, models: (data.data || []).map((item) => item.id).filter(Boolean).sort() };
}

async function imageAsDataUrl(imageUrl) {
  try {
    const url = new URL(imageUrl);
    const supported = url.protocol === 'https:' && (url.hostname.endsWith('.made-in-china.com') || url.hostname.endsWith('.vemic.com'));
...
function stripImageNames(value) { return String(value || '').replace(IMAGE_FILE_PATTERN, '[图片内容由视觉模型单独识别]').replace(/C:\\fakepath\\[^\s,;]+/gi, '[已移除图片文件名]'); }
function cleanEvidenceRows(rows) { return (rows || []).map(stripImageNames).filter(row => !/^[^：:]{0,30}[：:]\s*\[(?:图片内容|已移除图片文件名)/.test(row)); }
function sanitizeModelEvidence(result) {
  if (!result || typeof result !== 'object') return result;
  const hasFileName = value => /\.(?:jpe?g|png|gif|webp|bmp|svg|avif)(?:\?|\b)/i.test(String(value || '')) || /图片文件名|image filename/i.test(String(value || ''));
...
async function readUrlInAuthenticatedTab(targetUrl) {
  let url;
  try { url = new URL(targetUrl); } catch { return { ok: false, reason: '商品 URL 格式不正确' }; }
  const supported = url.protocol === 'https:' && (url.hostname === 'made-in-china.com' || url.hostname.endsWith('.made-in-china.com') || url.hostname === 'vemic.com' || url.hostname.endsWith('.vemic.com'));
  if (!supported) return { ok: false, reason: '仅支持 VEMIC / Made-in-China 的 HTTPS 商品 URL' };
  const tab = await chrome.tabs.create({ url: 'about:blank', active: false });
  try {
    const loadedPromise = new Promise((resolve, reject) => {
      const timer = setTimeout(() => { chrome.tabs.onUpdated.removeListener(listener); reject(new Error('URL 页面加载超时（30 秒）')); }, 30000);
      const listener = (tabId, info, changedTab) => { if (tabId === tab.id && info.status === 'complete') { clearTimeout(timer); chrome.tabs.onUpdated.removeListener(listener); resolve(changedTab); } };
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      ...
      try {
        ...
        if (fieldsResponse?.loginRequired) { bestResponse = fieldsResponse; break; }
        if (fieldsResponse?.fields) {
          const fields = fieldsResponse.fields;
          const score = (fields.title ? 20 : 0) + (fields.category ? 10 : 0) + Math.min(40, (fields.specs?.length || 0) * 2) + Math.min(20, (fields.formFields?.length || 0)) + Math.min(10, Math.floor((fields.visibleText?.length || 0) / 1000));
          if (score > bestScore) { bestScore = score; bestResponse = fieldsResponse; stableRounds = 0; } else stableRounds += 1;
          if (bestScore >= 45 && stableRounds >= 3) break;
        }
      }
      catch (error) { lastMessageError = error.message || String(error); }
    }
    fieldsResponse = bestResponse;
    if (fieldsResponse?.loginRequired) {
      await chrome.tabs.update(tab.id, { active: true });
      return { ok: false, loginRequired: true, loginUrl: loaded.url, reason: '登录会话已失效，请在打开的页面完成登录后重新读取 URL', keepTab: true };
    }
    if (!fieldsResponse?.fields) return { ok: false, reason: `页面已加载，但数据读取脚本未就绪${lastMessageError ? `：${lastMessageError}` : ''}` };
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message?.type === 'REQUEST_MIC_FIELDS') {
      ...
      } catch { return { ok: false, reason: 'CONTENT_SCRIPT_UNAVAILABLE' }; }
    }
    if (message?.type === 'OPEN_OPTIONS') { await chrome.runtime.openOptionsPage(); return { ok: true }; }
    if (message?.type === 'TEST_AI') {
      const out = await callAI([{ role: 'system', content: '请只输出 JSON。' }, { role: 'user', content: '输出 {"ok":true,"message":"连接成功"}' }], 100);
      return { ok: out.result?.ok === true, ...out };
    }
    if (message?.type === 'LIST_AI_MODELS') return { ok: true, ...(await listAIModels()) };
    if (message?.type === 'TRANSLATE_TEXT') {
      const sourceText = String(message.text || '').trim().slice(0, 12000);
      if (!sourceText) return { ok: false, reason: '没有可翻译的英文内容' };
      const out = await callAI([{ role: 'system', content: '你是专业跨境电商翻译。将用户提供的英文忠实翻译为简体中文，保留型号、数字、单位、品牌和段落结构。只输出 JSON：{"translation":"中文译文"}' }, { role: 'user', content: sourceText }], 1800);
      return { ok: true, translation: out.result?.translation || '', provider: out.provider, model: out.model };
    }
    if (message?.type === 'ANALYZE_PRODUCT') {
      const source = message.fields || {};
      const compactFields = { title: stripImageNames(source.title), category: stripImageNames(source.category), keywords: cleanEvidenceRows(source.keywords), specs: cleanEvidenceRows(source.specs).slice(0, 120), formFields: cleanEvidenceRows(source.formFields).slice(0, 120), certifications: cleanEvidenceRows(source.certifications), description: stripImageNames(source.description).slice(0, 5000), sku: stripImageNames(source.sku), brand: stripImageNames(source.brand), companyName: stripImageNames(source.companyName), companyProfile: stripImageNames(source.companyProfile).slice(0, 6000), visibleText: stripImageNames(source.visibleText).slice(0, 15000), imageCount: (source.images || []).length, frameCount: source.frameCount, url: source.url };
      ...
      const visionUrls = visionCapable ? await Promise.all((source.images || []).slice(0, 4).map(image => imageAsDataUrl(image.src))) : [];
      const imageBlocks = visionUrls.map(url => ({ type: 'image_url', image_url: { url } }));
      const userContent = imageBlocks.length ? [{ type: 'text', text: `请结合真实图片像素与以下页面字段完成诊断并输出 JSON。禁止根据图片文件名或 URL 猜测图片内容：\n${payload}` }, ...imageBlocks] : `请诊断以下商品页面数据并输出 JSON。当前模型未启用视觉能力，不得把图片 URL 当作图片证据：\n${payload}`;
      const out = await callAI([{ role: 'system', content: SYSTEM_PROMPT }, { role: 'user', content: userContent }]);
      out.visionUsed = imageBlocks.length > 0;
      return { ok: true, ...out };
    }
    return { ok: false, reason: 'UNKNOWN_MESSAGE' };
  })().then(sendResponse).catch(error => sendResponse({ ok: false, reason: error.message }));
  return true;
});
