const state = { tab: 0, subtab: 0, fields: null, report: null, loading: false, elapsed: 0, error: '', meta: null }
const esc = (v) =>
  String(v ?? '').replace(
    /[&<>'"]/g,
    (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' })[c],
...
function summary() {
  ...
  if (state.loading)
    e.innerHTML = `<b>AI 正在分析… ${state.elapsed} 秒</b><div class="loader"></div><div class="muted">K3 推理可能需要 60–125 秒，其他模型通常 10–40 秒</div>`
  else if (state.error)
    e.innerHTML = `<div class="danger">${esc(state.error)}</div><button class="pill-btn" data-action="settings">打开 API 设置</button>`
  else if (!state.report) e.innerHTML = '<b>读取商品 URL 后，点击“AI 分析商品”</b>'
  else {
...
function tabs() {
  ...
  document.getElementById('tabs').innerHTML = x
    .map(
      (n, i) =>
        `<button class="tab-btn ${state.tab === i ? 'active' : ''}" data-action="tab" data-i="${i}">${n}</button>`,
    )
    .join('')
}
function overview(r) {
...
function truth(r) {
  return `<div class="section-label">视觉与字段身份候选</div>${arr(r.identityCandidates)
    ...
    .join(
      '',
    )}<div class="card manual-identity"><b>以上都不符合</b><input id="manualIdentity" placeholder="输入正确的英文商品身份"><button data-action="confirm-manual-identity">确认并重新分析</button></div><div class="section-label">事实台账</div>${arr(
    r.facts,
  )
    .map(
      (x) =>
...
function content(r) {
  ...
  if (state.subtab === 0)
    body = arr(c.titles)
      ...
      .join('')
  else if (state.subtab === 1) body = detailView(c.detail)
  else if (state.subtab === 2)
    body = arr(c.faq)
      .map((x) => {
        const text = `Q: ${x.question}\nA: ${x.answer}`
  ...
  return `<div class="subtabs">${names.map((x, i) => `<button class="subtab-btn ${state.subtab === i ? 'active' : ''}" data-action="subtab" data-i="${i}">${x}</button>`).join('')}</div>${body}`
}
function debug(r) {
  const d = r.debug || {}
  return `<div class="card"><b>真实页面读取</b><p>标题：${state.fields?.title ? '已读取' : '未读取'}</p><p>类目：${state.fields?.category ? '已读取' : '未读取'}</p><p>规格/表格：${arr(state.fields?.specs).length} 条</p><p>表单字段：${arr(state.fields?.formFields).length} 条</p><p>页面/框架：${state.fields?.frameCount || 1} 个</p><p>正文证据：${state.fields?.visibleText?.length || 0} 字符</p><p>图片：${arr(state.fields?.images).length} 张</p></div><div class="card"><b>API 调用</b><p>提供商：${esc(state.meta?.provider || '—')}</p><p>模型：${esc(state.meta?.model || '—')}</p><p>请求次数：${esc(state.meta?.attempts || 1)}</p><p>Token：${esc(state.meta?.usage?.total_tokens || '—')}</p><p>读取地址：${esc(state.fields?.url || '—')}</p></div><div class="card"><b>缺失字段</b>${
    arr(d.missingFields)
      .map((x) => `<p>• ${esc(x)}</p>`)
      .join('') || '<p>无</p>'
...
function render() {
  ...
  if (!state.report)
    c.innerHTML = state.fields
      ? '<div class="card">商品 URL 数据已读取。点击下方按钮调用所选 AI 完成诊断。</div>'
      : '<div class="card">请粘贴商品 URL，或读取当前页 URL。</div>'
  else c.innerHTML = [overview, truth, keywords, content, debug][state.tab](state.report)
}
...
async function read() {
  ...
  state.error = r?.reason || '无法取得当前页 URL'
  state.loading = false
  render()
  return false
}
async function analyze() {
  ...
  try {
    ...
    } else state.error = r?.reason || 'AI 分析失败'
  } catch (error) {
    state.error = `AI 请求中断：${error.message || error}`
  } finally {
    clearInterval(timer)
    state.loading = false
    render()
...
document.getElementById('panel').addEventListener('click', async (e) => {
  ...
  if (!x) return
  if (x.dataset.action === 'tab') state.tab = Number(x.dataset.i)
  else if (x.dataset.action === 'subtab') state.subtab = Number(x.dataset.i)
  else if (x.dataset.action === 'settings') {
    chrome.runtime.sendMessage({ type: 'OPEN_OPTIONS' })
    return
  } else if (x.dataset.action === 'read-url') {
    await readUrl(document.getElementById('productUrl').value.trim())
    return
...
document.getElementById('productUrl').addEventListener('keydown', (e) => {
  if (e.key === 'Enter') readUrl(e.currentTarget.value.trim())
})
render()
read()
