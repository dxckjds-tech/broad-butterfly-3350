...
async function persist({validate=false,quiet=false}={}){
  ...
  if(validate){
    ...
    const activeKey=data.provider==='kimi'?(kimiKey||saved.kimiApiKey):(deepseekKey||saved.deepseekApiKey||saved.apiKey);
...
function scheduleSave(){
  ...
  saveTimer=setTimeout(()=>persist({quiet:true}).then(()=>$('status').textContent='API Key 已自动保存').catch(e=>$('status').textContent=e.message),350);
}

$('provider').addEventListener('change',()=>{toggle();persist({quiet:true});});
['deepseekModel','deepseekThinking','kimiModel','deepseekBaseUrl','kimiBaseUrl'].forEach(id=>$(id).addEventListener('change',()=>persist({quiet:true})));
['deepseekApiKey','kimiApiKey'].forEach(id=>$(id).addEventListener('input',scheduleSave));
$('save').addEventListener('click',()=>persist({validate:true}).catch(e=>$('status').textContent=e.message));
$('test').addEventListener('click',async()=>{try{await persist({validate:true});$('status').textContent='正在测试…';const r=await chrome.runtime.sendMessage({type:'TEST_AI'});$('status').textContent=r?.ok?`${r.provider} 连接成功 · ${r.model||''}`:`连接失败：${r?.reason||'未知错误'}`;}catch(e){$('status').textContent=e.message;}});
async function refreshModels(){try{await persist({validate:true,quiet:true});$('status').textContent='正在获取账号可用模型…';const r=await chrome.runtime.sendMessage({type:'LIST_AI_MODELS'});if(!r?.ok)throw new Error(r?.reason||'获取模型失败');const list=$('kimiModels');list.innerHTML='';r.models.forEach(id=>{const option=document.createElement('option');option.value=id;list.appendChild(option);});if(r.models.length&&!r.models.includes($('kimiModel').value)){$('kimiModel').value=r.models[0];await persist({quiet:true});}$('status').textContent=r.models.length?`已获取 ${r.models.length} 个可用模型，当前：${$('kimiModel').value}`:'账号未返回可用模型';}catch(e){$('status').textContent=`获取模型失败：${e.message}`;}}
